...
def initSituation(game):
    ...
    Either a 'pebbles' field exist in game, the its value is used, or
    default number of pebbles (defined in _DEFAULT_PEBBLES constant) is used.
...
def nextSituations(game, situation, player):
    ...
    In Nim game, the possible plays are taking 2 or 3 pebbles when possible,
    then next situations are situations with 2 or 3 pebbles less.
...
def getWinner(game, situation, player):
    """
    Gives the winner of the game that end in situation. There is a tie game when 1 pebble remains,
    else the previous player has taken the last pebble, then the current player wins.
...
def nextSituations(game, situation,player):
    ...
    res = []
    if situation >= 3 :
        res.append((situation-3,3))
        res.append((situation-2,2))
    elif situation == 2:
        res.append((situation-2,2))
    return res
def changeValue(situation, valueToPlay, comp):
    ...
    :type comp: Player
    """
    print('the computer took '+str(valueToPlay)+' pebbles')
    situation = situation - valueToPlay
    return situation
